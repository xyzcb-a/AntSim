...
def Main():
    ...
    while Choice != "9":
        ...
        if Choice == "1":
            ...
            EndRow = 0
            EndColumn = 0
            StartRow, StartColumn = GetCellReference(StartRow, StartColumn)
            EndRow, EndColumn = GetCellReference(EndRow, EndColumn)
            print(ThisSimulation.GetAreaDetails(StartRow, StartColumn, EndRow, EndColumn))
        elif Choice == "3":
            Row = 0
            Column = 0
            Row, Column = GetCellReference(Row, Column)
...
def GetCellReference(Rows, Columns):
    ...
    while not ValidRow:
        ...
        print()
        try:
            if Rows <= (Main.SimulationParameters[1]) and Rows >= 1:
                ValidRow = True 
            else: 
                print(f"That is not a valid height. It must be between 1 and {Main.SimulationParameters[1]}. Please try again.")
                print()
                continue
        except ValueError:
