    ...
    def AdvanceStage(self, NumberOfStages):
        for Count in range(1, NumberOfStages + 1):
            ...
            for N in self._Nests:
                ...
                print(self.GetNestSummary())
                
    def GetNestSummary(self):
        Info = ""
        for N in self._Nests:
            NestInfo = ""

            NestID = N.GetID()
            NestInfo += f"Nest {NestID} "

            NestFoodLevel = N.GetFoodLevel()
            NestInfo += f"Food level {NestFoodLevel} "

            AntsInNest = 0
            AntsInNestCarryingFood = 0
            FoodInTransit = 0

            NestRow = N.GetRow()
            NestColumn = N.GetColumn()

            for A in self._Ants:
                if A.GetNestRow() == NestRow and A.GetNestColumn() == NestColumn:
                    if isinstance(A, WorkerAnt):
                        AntsInNest += 1
                        if A.GetFoodCarried() > 0:
                            AntsInNestCarryingFood += 1
                            FoodInTransit += A.GetFoodCarried()

            
            NestInfo += f"Workers: {AntsInNest}, Carrying food {AntsInNestCarryingFood}, Food in transit: {FoodInTransit}"
